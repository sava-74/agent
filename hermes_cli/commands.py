...
class CommandDef:
    ...
    category: str                      # "Сессия", "Конфигурация", etc.
...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("new", "Начать новую сессию (новый ID + история)", "Сессия",
               aliases=("reset",), args_hint="[name]"),
    CommandDef("topic", "Включить или просмотреть топик-сессии Telegram", "Сессия",
               gateway_only=True, args_hint="[off|help|session-id]"),
    CommandDef("clear", "Очистить экран и начать новую сессию", "Сессия",
               cli_only=True),
    CommandDef("redraw", "Принудительно перерисовать UI (восстанавливает после сбоев терминала)", "Сессия",
               cli_only=True),
    CommandDef("history", "Показать историю разговора", "Сессия",
               cli_only=True),
    CommandDef("save", "Сохранить текущий разговор", "Сессия",
               cli_only=True),
    CommandDef("retry", "Повторить последнее сообщение (отправить агенту повторно)", "Сессия"),
    CommandDef("undo", "Удалить последний обмен пользователь/ассистент", "Сессия"),
    CommandDef("title", "Установить заголовок для текущей сессии", "Сессия",
               args_hint="[name]"),
    CommandDef("handoff", "Передать эту сессию в платформу сообщений (Telegram, Discord и т.д.)", "Сессия",
               args_hint="<platform>", cli_only=True),
    CommandDef("branch", "Ответвить текущую сессию (исследовать другой путь)", "Сессия",
               aliases=("fork",), args_hint="[name]"),
    CommandDef("compress", "Вручную сжать контекст разговора", "Сессия",
               args_hint="[focus topic]"),
    CommandDef("rollback", "Список или восстановление чекпоинтов файловой системы", "Сессия",
               args_hint="[number]"),
    CommandDef("snapshot", "Создать или восстановить снимки состояния конфигурации/состояния Hermes", "Сессия",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "Убить все запущенные фоновые процессы", "Сессия"),
    CommandDef("approve", "Одобрить ожидающую опасную команду", "Сессия",
               gateway_only=True, args_hint="[session|always]"),
    CommandDef("deny", "Отклонить ожидающую опасную команду", "Сессия",
               gateway_only=True),
    CommandDef("background", "Запустить запрос в фоновом режиме", "Сессия",
               aliases=("bg", "btw"), args_hint="<prompt>"),
    CommandDef("agents", "Показать активные агенты и запущенные задачи", "Сессия",
               aliases=("tasks",)),
    CommandDef("queue", "Поставить запрос в очередь для следующего хода (не прерывает)", "Сессия",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "Вставить сообщение после следующего вызова инструмента без прерывания", "Сессия",
               args_hint="<prompt>"),
    CommandDef("goal", "Установить постоянную цель, над которой Hermes работает в нескольких ходах до достижения", "Сессия",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("subgoal", "Добавить или управлять дополнительными критериями для активной цели", "Сессия",
               args_hint="[text | remove N | clear]"),
    CommandDef("status", "Показать информацию о сессии", "Сессия"),
    CommandDef("whoami", "Показать доступ к командам (админ / пользователь)", "Информация"),
    CommandDef("profile", "Показать активный профиль и домашний каталог", "Информация"),
    CommandDef("sethome", "Установить этот чат как домашний канал", "Сессия",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "Возобновить ранее названную сессию", "Сессия",
               args_hint="[name]"),

    # Configuration
    CommandDef("sessions", "Просмотр и возобновление предыдущих сессий", "Сессия"),

    # Configuration
    CommandDef("config", "Показать текущую конфигурацию", "Конфигурация",
               cli_only=True),
    CommandDef("model", "Переключить модель для этой сессии", "Конфигурация",
               aliases=("provider",), args_hint="[model] [--provider name] [--global]"),
    CommandDef("codex-runtime", "Переключить среду выполнения codex app-server для моделей OpenAI/Codex",
               "Конфигурация", aliases=("codex_runtime",),
               args_hint="[auto|codex_app_server]"),
    CommandDef("gquota", "Показать использование квоты Google Gemini Code Assist", "Информация",
               cli_only=True),

    CommandDef("personality", "Установить предопределённую личность", "Конфигурация",
               args_hint="[name]"),
    CommandDef("statusbar", "Переключить строку состояния контекста/модели", "Конфигурация",
               cli_only=True, aliases=("sb",)),
    CommandDef("verbose", "Переключить отображение прогресса инструментов: off -> new -> all -> verbose",
               "Конфигурация", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "Переключить футер метаданных шлюза на финальных ответах",
               "Конфигурация", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "Переключить режим YOLO (пропуск всех одобрений опасных команд)",
               "Конфигурация"),
    CommandDef("reasoning", "Управление усилием рассуждения и отображением", "Конфигурация",
               args_hint="[level|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "Toggle fast mode — OpenAI Priority Processing / Anthropic Fast Mode (Normal/Fast)", "Конфигурация",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "Показать или сменить скин/тему отображения", "Конфигурация",
               cli_only=True, args_hint="[name]"),
    CommandDef("indicator", "Выбрать стиль индикатора занятости TUI", "Конфигурация",
               cli_only=True, args_hint="[kaomoji|emoji|unicode|ascii]",
               subcommands=("kaomoji", "emoji", "unicode", "ascii")),
    CommandDef("voice", "Переключить голосовой режим", "Конфигурация",
               args_hint="[on|off|tts|status]", subcommands=("on", "off", "tts", "status")),
    CommandDef("busy", "Управление действием Enter во время работы Hermes", "Конфигурация",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Tools & Skills
    CommandDef("tools", "Управление инструментами: /tools [list|disable|enable] [name...]", "Инструменты и навыки",
               args_hint="[list|disable|enable] [name...]", cli_only=True),
    CommandDef("toolsets", "Список доступных наборов инструментов", "Инструменты и навыки",
               cli_only=True),
    CommandDef("skills", "Поиск, установка, проверка или управление навыками",
               "Инструменты и навыки", cli_only=True,
               subcommands=("search", "browse", "inspect", "install")),
    CommandDef("cron", "Управление запланированными задачами", "Инструменты и навыки",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("curator", "Фоновое обслуживание навыков (статус, запуск, закрепление, архивирование, список)",
               "Инструменты и навыки", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore", "list-archived")),
    CommandDef("kanban", "Доска многопрофильного сотрудничества (задачи, ссылки, комментарии)",
               "Инструменты и навыки", args_hint="[subcommand]",
    ...
    CommandDef("reload", "Перезагрузить переменные .env в текущую сессию", "Инструменты и навыки",
               cli_only=True),
    CommandDef("reload-mcp", "Перезагрузить MCP серверы из конфигурации", "Инструменты и навыки",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Повторное сканирование ~/.hermes/skills/ на новые или удалённые навыки",
               "Инструменты и навыки", aliases=("reload_skills",)),
    CommandDef("browser", "Подключить инструменты браузера к живому Chrome через CDP", "Инструменты и навыки",
    ...
    # Info
    CommandDef("commands", "Просмотр всех команд и навыков (постранично)", "Информация",
               gateway_only=True, args_hint="[page]"),
    CommandDef("help", "Показать доступные команды", "Информация"),
    CommandDef("restart", "Грациозно перезапустить шлюз после завершения активных выполнений", "Сессия",
               gateway_only=True),
    CommandDef("usage", "Показать использование токенов и лимиты для текущей сессии", "Информация"),
    CommandDef("insights", "Показать аналитику и инсайты использования", "Информация",
               args_hint="[days]"),
    CommandDef("platforms", "Показать статус платформы шлюза/сообщений", "Информация",
               cli_only=True, aliases=("gateway",)),
    CommandDef("platform", "Приостановить, возобновить или показать статус платформы шлюза", "Информация",
               gateway_only=True, args_hint="<pause|resume|list> [name]"),
    CommandDef("copy", "Копировать последний ответ ассистента в буфер обмена", "Информация",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "Прикрепить изображение из буфера обмена", "Информация",
               cli_only=True),
    CommandDef("image", "Прикрепить локальный файл изображения для следующего запроса", "Информация",
               cli_only=True, args_hint="<path>"),
    CommandDef("update", "Обновить Hermes Agent до последней версии", "Информация",
               gateway_only=True),
    CommandDef("debug", "Загрузить отчёт отладки (системная информация + логи) и получить ссылки", "Информация"),

    # Exit
    CommandDef("quit", "Выйти из CLI (используйте --delete для удаления истории сессии)", "Выход",
